"""
AWQ Heuristic v2 (awq_h2.py)

Key Difference from awq_ho.py:
- awq_ho.py: Uses heuristic in BOTH grid search AND final quantization
- awq_h2.py: Uses heuristic ONLY in FINAL quantization, NOT in grid search

Strategy:
1. Grid search for best α: Always uses standard quantization (like gw_awq_asym_l2.py)
2. Final quantization: Uses heuristic if enabled

This is more efficient and may give better results since α is optimized for standard quantization.
"""
...
class AWQHeuristicV2Quantizer:
    """
    AWQ Quantizer with Heuristic ONLY in Final Quantization.

    Grid search: Always uses standard quantization
    Final quantization: Uses heuristic if enabled
    """
    ...
    def __init__(self, model, tokenizer, device="cuda", bits=4, n_grid=20,
        ...
        print(f"\n[AWQ Heuristic V2 Quantizer Initialized]")
        ...
        if use_heuristic:
            ...
            print(f"  Strategy: Heuristic ONLY in final quantization (NOT in grid search)")
    ...
    def search_best_scale(self, name, module):
        ...
        KEY DIFFERENCE: Always uses standard quantization (NO heuristic)
        This is the same for both use_heuristic=True and use_heuristic=False
        """
        if name not in self.activation_data or len(self.activation_data[name]) == 0:
        ...
        for grid_idx in range(self.n_grid + 1):
            ...
            # ALWAYS use standard quantization in grid search (NO heuristic)
    ...
    def quantize_layer(self, name, module):
        ...
        KEY DIFFERENCE: After finding best scale with standard quantization,
        applies heuristic in FINAL quantization if enabled.
        """
        # Grid search uses standard quantization (always)
        ...
        # Store metadata
        # Note: 'error' is from grid search (always standard quantization)
        # The heuristic is applied AFTER finding best α, so error reflects standard quantization
        self.layer_scales[name] = {
            'scales': best_scales.cpu(),
            'alpha': best_alpha,
            'error': best_error  # Error from grid search (standard quantization)
        }

        # Free memory
        del W, W_scaled, W_quant, W_final, best_scales
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    # ========== HEURISTIC QUANTIZATION (only used in FINAL quantization when use_heuristic=True) ==========
    ...
    def quantize_weight_heuristic_groupwise(self, W, group_activation_means):
        ...
        Only called in FINAL quantization when use_heuristic=True.
    ...
    def quantize_model(self):
        ...
        print("\n" + "=" * 80)
        if self.use_heuristic:
            print("Quantizing with AWQ Heuristic V2 (Heuristic in FINAL quantization ONLY)")
        else:
            print("Quantizing with AWQ WITHOUT Heuristic (EXACT as gw_awq_asym_l2.py)")
        print("=" * 80)
        print("Method:")
        print("  1. Compute per-input-channel L2 salience: s[j] = E[X[:, j]²]")
        print("  2. Grid search for optimal α ∈ [0, 1]")
        print("     → ALWAYS uses standard quantization (NO heuristic in grid search)")
        print("  3. Scale weight columns: W[:, j] *= s[j]^α")
        print(f"  4. GROUP-WISE ASYMMETRIC INT4 quantization [0, 15]")
        if self.use_heuristic:
            print("     → FINAL quantization WITH global greedy rounding correction")
            ...
            try:
                ...
                if quantized_count % 10 == 0 and quantized_count > 0:
                    if name in self.layer_scales:
                        ...
                        print(f"\n  Layer {name}:")
                        print(f"    α={info['alpha']:.3f}, grid_search_error={info['error']:.6f}")
                        if self.use_heuristic:
                            print(f"    (Note: Error is from grid search; heuristic applied in final quantization)")
...
def main():
    parser = argparse.ArgumentParser(
        description="AWQ Heuristic V2 (Heuristic ONLY in final quantization) for MiniCPM-2B",
    ...
    parser.add_argument("--use-heuristic", action="store_true", help="Enable heuristic rounding in FINAL quantization")
    ...
    if args.output_dir is None:
        if args.use_heuristic:
            args.output_dir = "./quantized_models/minicpm_awq_h2_heuristic"
        else:
            args.output_dir = "./quantized_models/minicpm_awq_h2_standard"
    ...
    print("=" * 80)
    print("AWQ Heuristic V2 (awq_h2.py)")
    print("=" * 80)
    print("Algorithm:")
    print("  1. Per-input-channel L2 salience: s[j] = E[X[:, j]²]")
    print("  2. Grid search optimal α ∈ [0, 1]")
    print("     → ALWAYS uses standard quantization (NO heuristic)")
    ...
    if args.use_heuristic:
        print("     → FINAL quantization: Global greedy rounding correction")
        print(f"     → WITH outlier masking: Top {args.outlier_percent*100:.1f}% ignored")
    else:
        print("     → FINAL quantization: Standard rounding (same as gw_awq_asym_l2.py)")
    ...
    print(f"Heuristic mode: {'ENABLED (final only)' if args.use_heuristic else 'DISABLED'}")
    ...
    quantizer = AWQHeuristicV2Quantizer(
        model=model,
        tokenizer=tokenizer,
        device=device,
        bits=4,
    ...
    print("\nAWQ Heuristic V2 Approach:")
    print("  ✓ L2 salience: E[X²] (MSE-aligned)")
    print("  ✓ Grid search for optimal α (ALWAYS standard quantization)")
    print("  ✓ Column-wise weight scaling")
    print(f"  ✓ GROUP-WISE ASYMMETRIC quantization [0, 15]")
    if args.use_heuristic:
        print("  ✓ FINAL quantization: Global greedy rounding")
        print(f"  ✓ WITH outlier masking: Top {args.outlier_percent*100:.1f}%")
    else:
        print("  ✓ FINAL quantization: Standard rounding")
    print("=" * 80)
