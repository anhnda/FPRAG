...
class StandardHeuristicAWQQuantizerWithRegularization:
    """
    Standard Heuristic AWQ with REGULARIZATION to prevent overfitting.

    KEY IMPROVEMENT: Adds penalty term to discourage excessive flips.
    - Objective: minimize |error - cumsum| + lambda * num_flips
    - Prevents overfitting to calibration data (WikiText-2)
    - Should improve in-distribution performance while maintaining cross-dataset gains

    This version uses the SAME base features as gw_awq_asym_l2.py, but adds
    heuristic-guided quantization with regularization on top.
    ...
    NEW: Regularization
    - Penalty term: lambda * num_flips
    - Prevents marginal flips that hurt generalization
    """

    def __init__(self, model, tokenizer, device="cuda", bits=4, n_grid=20,
                 group_size=128, use_heuristic=True, outlier_percent=0.05, reg_lambda=0.001):
        self.model = model
        ...
        print(f"\n[Standard Heuristic AWQ Quantizer with REGULARIZATION Initialized]")
        ...
        if use_heuristic:
            ...
            print(f"  Objective: minimize |error| + {reg_lambda} * num_flips")
    ...
    def quantize_weight_heuristic_groupwise(self, W, group_activation_means, apply_heuristic=True):
        ...
        # Find Best K with REGULARIZATION
        # NEW: minimize |error - cumsum| + lambda * num_flips
        ...
        # Add regularization penalty: lambda * k for k flips
        # This discourages excessive flips and prevents overfitting
        if apply_heuristic and self.reg_lambda > 0:
            # Create penalty: [0, 1, 2, ..., padded_in_features] * lambda
            num_flips_penalty = torch.arange(padded_in_features + 1, device=device, dtype=torch.float32)
            # Expand to [out_features, padded_in_features + 1]
            num_flips_penalty = num_flips_penalty.unsqueeze(0).expand(out_features, -1)
            # Add penalty to residuals
            all_residuals = all_residuals + self.reg_lambda * num_flips_penalty
    ...
    def quantize_layer(self, name, module):
        ...
        apply_heuristic_this_layer = self.use_heuristic
        ...
        W_quant = self.quantize_weight_heuristic_groupwise(
            W_scaled,
            scaled_act_mean,
            apply_heuristic=apply_heuristic_this_layer
        )
    ...
    def quantize_model(self):
        print("\n" + "=" * 80)
        print("Quantizing with Standard Heuristic AWQ + REGULARIZATION")
        print(f"Regularization lambda: {self.reg_lambda}")
        print("Objective: minimize |error| + lambda * num_flips")
        print("=" * 80)
...
def main():
    ...
    parser.add_argument("--reg-lambda", type=float, default=0.001,
                       help="Regularization strength (default: 0.001). Higher = fewer flips")
    parser.add_argument("--output-dir", type=str, default="./quantized_models/minicpm_awq_sh_reg")
    ...
    quantizer = StandardHeuristicAWQQuantizerWithRegularization(
        model=model,
        tokenizer=tokenizer,
        device=device,
        bits=4,
