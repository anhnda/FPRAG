...
2. BOS handling: Family-aware.
   - Llama 3 (PreTrainedTokenizerFast, bos=128000, eos=128001): inject BOS once.
   - Llama 2 / Mistral (LlamaTokenizerFast, bos=1, eos=2):       inject BOS once.
   - Qwen2 / Qwen2.5 / Qwen3 (Qwen2TokenizerFast, bos=None):     no BOS injection.
   - GPT-2 family (bos==eos==<|endoftext|>):                     no BOS injection.
...
def should_inject_bos(tokenizer) -> bool:
    """
    Decide whether to manually prepend BOS for stream PPL evaluation.

    Returns True only when the model has a *distinct* BOS that the tokenizer
    omits (because we called it with add_special_tokens=False).

    Cases:
      - Llama 3 / 3.1 / 3.2:     bos=128000, eos=128001            -> True
      - Llama 2 / Mistral:       bos=1,      eos=2                 -> True
      - Qwen2 / Qwen2.5 / Qwen3: bos=None                          -> False
      - GPT-2:                   bos==eos==<|endoftext|> (50256)   -> False
    """
    bos = tokenizer.bos_token_id
    eos = tokenizer.eos_token_id
    if bos is None:
        return False  # Qwen family
    if bos == eos:
        return False  # GPT-2 style: injecting <|endoftext|> at position 0 hurts stream PPL
    return True
...
class AWQSlidingWindowValidator:
    ...
    def evaluate_sliding_window(self, model, tokenizer, texts):
        ...
        inject_bos = should_inject_bos(tokenizer)
        print(f"  Tokenizer class: {type(tokenizer).__name__}")
        print(f"  BOS={tokenizer.bos_token_id}  EOS={tokenizer.eos_token_id}  inject_bos={inject_bos}")

        for text in texts:
            # Tokenize WITHOUT special tokens — we control BOS ourselves below.
            encodings = tokenizer(text, return_tensors="pt", add_special_tokens=False)
            input_ids = encodings.input_ids
            print(f"  First 10 token IDs (pre-BOS): {input_ids[0, :10].tolist()}")

            # Family-aware BOS injection
            if inject_bos:
                bos_id = tokenizer.bos_token_id
                if input_ids.shape[1] == 0 or input_ids[0, 0].item() != bos_id:
                    bos_tensor = torch.tensor([[bos_id]], device=input_ids.device)
                    input_ids = torch.cat([bos_tensor, input_ids], dim=1)
                    print(f"  Injected BOS={bos_id} at position 0")
            ...
            for begin_loc in pbar:
                ...
                evaluated_tokens += trg_len

                if nlls:
                    current_nll = torch.stack(nlls).sum()
                    current_ppl = torch.exp(current_nll / (total_tokens + evaluated_tokens)).item()
                    pbar.set_postfix({
                        "PPL": f"{current_ppl:.4f}",
                        "tokens": f"{total_tokens + evaluated_tokens:,}",
                    })
    ...
    def load_tokenizer(model_path):
        ...
        Works across:
          - Llama 3.x      -> PreTrainedTokenizerFast (no LlamaTokenizerFast for L3)
          - Llama 2 / Mistral -> LlamaTokenizerFast
          - Qwen2 / 2.5 / 3   -> Qwen2TokenizerFast
    ...
    def evaluate_model_on_dataset(self, model_path, model_name, texts, dataset_name):
        ...
        try:
            ...
            # Qwen2.5 ships a proper pad_token (<|endoftext|>); this fallback is
            # only used by tokenizers that don't, e.g. some Llama checkpoints.
