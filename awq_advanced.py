...
class AdvancedHeuristicAWQQuantizer:
    """
    Advanced AWQ with Heuristic-Guided Asymmetric Quantization.

    Combines best features from awq_op_ref.py with improvements:
    - float64 precision for activation stats (from awq_op_ref.py)
    - Sequential sampling in grid search (from awq_op_ref.py)
    - bfloat16 model loading (from awq_op_ref.py)
    - Heuristic in BOTH grid search AND final quantization (from awq_op_ref.py)
    - Memory optimizations (from awq_h2.py)
    - OOM protection for large layers (from awq_h2.py)
    """
    ...
    def __init__(self, model, tokenizer, device="cuda", bits=4, n_grid=20, 
        ...
        print(f"\n[Advanced Heuristic AWQ Quantizer Initialized]")
        ...
        print(f"  Features: float64 stats, sequential sampling, bfloat16 model, OOM protection")
    ...
    def quantize_layer(self, name, module):
        """Apply Heuristic Group-Wise AWQ Quantization with OOM protection."""
        ...
        if self.use_heuristic:
            ...
            max_layer_size_gb = 0.5

            if layer_size_gb > max_layer_size_gb:
                print(f"\n  ⚠️  Layer {name} too large ({layer_size_gb:.2f}GB), skipping heuristic")
                W_quant = self.quantize_weight_heuristic_groupwise(
                    W_scaled, scaled_act_mean, apply_heuristic=False
                )
            else:
                W_quant = self.quantize_weight_heuristic_groupwise(
                    W_scaled, scaled_act_mean, apply_heuristic=True
                )
        else:
            W_quant = self.quantize_weight_heuristic_groupwise(
                W_scaled, scaled_act_mean, apply_heuristic=False
        ...
        del best_scales, scaled_act_mean, W_scaled, W_quant, W_final, W
    ...
    def quantize_model(self):
        print("\n" + "=" * 80)
        print("Quantizing with Advanced Heuristic AWQ")
        print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Advanced AWQ: Best of awq_op_ref.py + Memory Optimizations"
    )
    ...
    parser.add_argument("--output-dir", type=str, default="./quantized_models/minicpm_awq_advanced",
    ...
    print("=" * 80)
    print("ADVANCED AWQ QUANTIZER")
    print("=" * 80)
    print("Winning features from awq_op_ref.py:")
    print("  ✓ float64 precision for activation statistics")
    print("  ✓ Sequential sampling in grid search (not random)")
    print("  ✓ bfloat16 model loading")
    print("  ✓ Heuristic in BOTH grid search AND final quantization")
    print("  ✓ activation_salience + 1e-6 (avoid division by zero)")
    print("\nAdded improvements:")
    print("  ✓ OOM protection for large layers (skips heuristic if > 0.5GB)")
    ...
    print(f"Outlier percent: {args.outlier_percent * 100:.1f}%")
    ...
    quantizer = AdvancedHeuristicAWQQuantizer(
        model=model,
        tokenizer=tokenizer,
        device=device,
        bits=4,
        ...
        use_heuristic=True  # Always use heuristic in advanced version
    )
    ...
    print("\n" + "=" * 80)
    print("✅ ADVANCED AWQ QUANTIZATION COMPLETE!")
    print("=" * 80)
    ...
    print("  ✓ Heuristic in grid search + final")
