"""
Group-Wise AWQ Implementation with ASYMMETRIC Quantization + L2 Salience + Knee-Point Weight Clamping (v2)

Key Difference from gw_awq_asym_l2.py:
- gw_awq_asym_l2.py: No weight clamping, uses full weight range
- gw_awq_clamp_l2.py: Uses Kneedle algorithm on SECOND HALF to find where importance flattens,
                       then clamps weights to range of top important weights (beginning to knee)

Algorithm (v2 - SECOND HALF KNEE):
...
2. For each layer (global knee detection, NOT per-channel):
   ...
   c. Sort by importance descending
   d. Run Kneedle on SECOND HALF (data[half:]) to find where importance flattens
   e. Convert knee to absolute index: knee_abs = half + knee_relative
   f. Compute k_offset = int(k_offset_ratio × num_weights) (default: 0%)
   g. Get weight range per channel: [min(W_top), max(W_top)] where W_top = top (knee+k_offset) weights
   h. Clamp all weights in each channel to their respective range
...
Key Innovation (v2):
- Run Kneedle on second half [half:] to find where tail flattens to ~0
- Clamp from beginning to knee (preserve important weights)
- Ignore unimportant tail after knee
"""
...
class GroupWiseAWQClampL2Quantizer:
    ...
    def compute_weight_clamp_ranges(self, W, activation_salience):
        ...
        2. Run Kneedle on SECOND HALF (data[half:]) to find where importance flattens to ~0
        3. Convert knee to absolute index: knee_abs = half + knee_relative
        4. Clamp from BEGINNING TO KNEE (the important weights)
        5. Ignore unimportant tail after knee

        Rationale:
        - Running Kneedle on second half [half:] finds where tail flattens to near-zero
        - Only preserve range of important weights (beginning to knee)
        - Tail weights are insignificant, so ignore them in range calculation
        - "Don't care about clearly non-significant weights"
        ...
        # NEW: Find knee on SECOND HALF (run Kneedle on data[half:])
        # This finds where importance flattens to ~0 in the tail region
        half_n = in_features // 2
        second_half_start = half_n
        second_half_len = in_features - second_half_start

        try:
            # Extract SECOND HALF of sorted importance for Kneedle
            x_range = np.arange(second_half_len)  # Relative indices [0, 1, 2, ...]
            y_values = sorted_importance_global[second_half_start:].cpu().numpy()  # Data from [half:]
            ...
            if valid_mask.sum() > 10:
                ...
                if knee_locator.knee is not None:
                    knee_relative = int(knee_locator.knee)  # Relative to second half
                    global_knee = second_half_start + knee_relative  # Convert to absolute index
                else:
                    # Fallback: use middle (50% of features)
            ...
            try:
                ...
                if quantized_count % 10 == 0 and quantized_count > 0:
                    if name in self.layer_scales:
                        ...
                        if clamp_info:
                            print(f"    knee={clamp_info.get('mean_knee', 0):.0f} ({clamp_info.get('knee_percentage', 0):.1f}%), "
...
def main():
    ...
    parser.add_argument("--k-offset-ratio", type=float, default=0.0,
                       help="K-offset ratio for knee point (default: 0%% of weights)")
