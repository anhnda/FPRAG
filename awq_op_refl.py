...
class HeuristicGroupWiseAWQQuantizerLeastFirst:
    """
    Group-Wise AWQ with Heuristic-Guided Asymmetric Quantization (LEAST-FIRST strategy).

    NEW STRATEGY: Flip LEAST significant weights first (far from rounding boundary)

    Logic:
    - Includes Outlier Masking (ignores top X% activations for stability)
    - Global Candidate Sorting based on Rounding Cost
    - CHANGE: Flips from least significant (low cost) to most significant (high cost)
    - Rationale: Weights far from boundary may be safer to flip
    """
    ...
    def __init__(self, model, tokenizer, device="cuda", bits=4, n_grid=20, 
        ...
        print(f"\n[Heuristic Group-Wise AWQ Quantizer (LEAST-FIRST) Initialized]")
        print(f"  Target bits: {bits}")
        print(f"  Group size: {group_size}")
        print(f"  Outlier protection: Top {outlier_percent*100:.1f}% ignored")
        print(f"  Strategy: FLIP LEAST SIGNIFICANT WEIGHTS FIRST")
    ...
    def quantize_weight_heuristic_groupwise(self, W, group_activation_means, apply_heuristic=True):
        ...
        # --- 4. Sorting & Optimization (LEAST-FIRST STRATEGY) ---

        # Calculate Cost: Distance to rounding boundary.
        # NEW STRATEGY: Sort Ascending. Low cost (0.01) = far from boundary = flip first.
        # Rationale: Weights already well-rounded may be safer to flip.
        rounding_costs = (W_div + zp_flat - W_int).abs()

        # Vectorization Trick: Set cost of INVALID candidates to 999.0.
        # Since we sort Ascending, valid costs (0.0 to 0.5) will come first.
        # Invalid ones (999.0) will go to the end.
        rounding_costs_masked = rounding_costs.clone()
        rounding_costs_masked[~valid_mask] = 999.0

        # Sort [out, in] independent for each row - ASCENDING (least significant first)
        sorted_indices = torch.argsort(rounding_costs_masked, dim=1, descending=False)
    ...
    def quantize_model(self):
        print("\n" + "=" * 80)
        print("Quantizing with Heuristic-Guided Group-Wise AWQ (LEAST-FIRST)")
        print("Strategy: Flip LEAST significant weights first")
        print("=" * 80)
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./quantized_models/minicpm_awq_op_refl")
    ...
    quantizer = HeuristicGroupWiseAWQQuantizerLeastFirst(
        model=model,
        tokenizer=tokenizer,
        device=device,
        bits=4,
