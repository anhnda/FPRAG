"""
Group-Wise AWQ Implementation with ASYMMETRIC Quantization

This implements AWQ with GROUP-WISE ASYMMETRIC quantization.

Key Difference from gw_awq.py:
- gw_awq.py: Symmetric quantization, INT4 range [-8, 7], zero_point=0
- gw_awq_asym.py: Asymmetric quantization, INT4 range [0, 15], zero_point computed

Asymmetric Quantization Benefits:
- Uses full INT4 range [0, 15] (16 values) vs symmetric [-8, 7] (16 values)
- Better for asymmetric weight distributions (e.g., post-ReLU, biased weights)
- Computes both scale AND zero_point per group
- More expressive but slightly more compute overhead
...
class GroupWiseAWQAsymmetricQuantizer:
    """
    Group-Wise AWQ with Asymmetric Quantization.
    ...
    def __init__(self, model, tokenizer, device="cuda", bits=4, n_grid=20, group_size=128):
        ...
        print(f"\n[Group-Wise AWQ ASYMMETRIC Quantizer Initialized]")
        print(f"  Target bits: {bits}")
        print(f"  Grid search points: {n_grid}")
        print(f"  Group size: {group_size}")
        print(f"  Quantization: GROUP-WISE ASYMMETRIC [0, 15]")
        print(f"  Advantage: Full INT4 range, better for asymmetric distributions")
    ...
    def quantize_weight_groupwise_asymmetric(self, W):
        ...
        Key Difference from Symmetric:
        - Symmetric: scale = abs_max / 7, range [-8, 7], zero_point = 0
        - Asymmetric: scale = (max - min) / 15, range [0, 15], zero_point computed
    ...
    def search_best_scale(self, name, module):
        ...
        Group-Wise AWQ Algorithm with Asymmetric Quantization:
    ...
    def quantize_layer(self, name, module):
        """
        Apply Group-Wise AWQ with Asymmetric Quantization to a linear layer.
    ...
    def quantize_model(self):
        """Quantize all linear layers using Group-Wise AWQ with Asymmetric Quantization."""
        print("\n" + "=" * 80)
        print("Quantizing with Group-Wise AWQ ASYMMETRIC Algorithm")
        print("=" * 80)
        print("Method:")
...
def main():
    parser = argparse.ArgumentParser(
        description="Group-Wise AWQ with ASYMMETRIC quantization for MiniCPM-2B",
    ...
    parser.add_argument("--output-dir", type=str, default="./quantized_models/minicpm_gw_awq_asym",
    ...
    print("=" * 80)
    print("Group-Wise AWQ with ASYMMETRIC Quantization")
    print("=" * 80)
    print("Algorithm:")
    ...
    print("\nKey Advantage: Full INT4 range, better for asymmetric weight distributions")
    print("=" * 80)
    ...
    quantizer = GroupWiseAWQAsymmetricQuantizer(
        model=model,
        tokenizer=tokenizer,
        device=device,
        bits=4,
    ...
    print("\nGroup-Wise AWQ ASYMMETRIC Approach:")
    ...
    print(f"  ✓ GROUP-WISE ASYMMETRIC quantization [0, 15] (group_size={args.group_size})")
    print("  ✓ Better representation of asymmetric weight distributions")
    print("\nNext Steps:")
