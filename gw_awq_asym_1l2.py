"""
Group-Wise AWQ Implementation with ASYMMETRIC Quantization + Modified L2 Salience

Key Difference from gw_awq_asym_l2.py:
- gw_awq_asym_l2.py: Uses E[X²] (standard L2 norm) for activation salience
- gw_awq_asym_1l2.py: Uses E[1+X²] (baseline-shifted L2) for activation salience

Why E[1+X²] Might Be Better:
- Prevents zero salience: E[X²] can be 0 for channels with zero activations
- Baseline protection: Even inactive channels get minimum salience of 1.0
- Better handling of small activations: Channels with tiny but non-zero values won't be ignored
- Prevents division by zero in scaling: s^α >= 1 for all channels

Algorithm:
1. Compute per-input-channel salience: s[j] = E[1 + X[:, j]²] (baseline-shifted L2)
...
class GroupWiseAWQAsymmetric1L2Quantizer:
    """
    Group-Wise AWQ with Asymmetric Quantization and Baseline-Shifted L2 Salience.

    Key Features:
    - Per-input-channel scaling based on E[1+X²] (baseline-shifted L2 salience)
    - Grid search for optimal scaling exponent α
    - GROUP-WISE ASYMMETRIC INT4 quantization [0, 15]
    - Prevents zero salience for inactive channels
    """
    ...
    def __init__(self, model, tokenizer, device="cuda", bits=4, n_grid=20, group_size=128):
        ...
        print(f"\n[Group-Wise AWQ ASYMMETRIC 1+L2 Quantizer Initialized]")
        print(f"  Target bits: {bits}")
        print(f"  Grid search points: {n_grid}")
        print(f"  Group size: {group_size}")
        print(f"  Quantization: GROUP-WISE ASYMMETRIC [0, 15]")
        print(f"  Salience metric: E[1+X²] (baseline-shifted L2 norm)")
    ...
    @torch.no_grad()
    def get_activation_salience_1l2(self, name):
        """
        Compute per-input-channel activation salience using baseline-shifted L2: E[1 + X[:, j]²]

        Key Difference from Standard L2:
        - Standard L2: E[X²] - Can be zero for inactive channels
        - Baseline-shifted: E[1+X²] - Minimum salience of 1.0 for all channels

        Why This is Better:
        - Prevents zero salience: All channels have at least baseline importance
        - Safer scaling: s^α >= 1 for all α ∈ [0, 1], avoiding division by tiny values
        - Small activation protection: Channels with tiny activations get baseline + small boost

        Mathematical intuition:
        - Quantization error ∝ weight magnitude × input magnitude
        - Even "inactive" channels need baseline protection from quantization
        - The +1 baseline represents the minimum risk for any channel

        Returns:
            Tensor of shape [in_features], each value >= 1.0
        """
        ...
        # Accumulate baseline-shifted L2 salience on CPU
        salience_sum = torch.zeros(in_features)

        for x in X_list:
            x_flat = x.reshape(-1, x.shape[-1])
            # KEY CHANGE: Add 1.0 baseline to each squared value
            salience_sum += (1.0 + x_flat.pow(2)).sum(dim=0)

        salience = salience_sum / total_samples

        # Verify: all salience values should be >= 1.0
        assert salience.min() >= 1.0, f"Unexpected: min salience = {salience.min()}"
    ...
    def search_best_scale(self, name, module):
        """
        Grid search for optimal per-input-channel scaling factor using baseline-shifted L2 salience.

        Algorithm with E[1+X²]:
        1. For α in [0, 0.05, 0.1, ..., 0.95, 1.0]:
           a. Compute scales: s[j] = (E[1 + X[:, j]²])^α for each input channel j
        ...
        # Get baseline-shifted L2 activation salience
        activation_salience = self.get_activation_salience_1l2(name)
        ...
        for grid_idx in range(self.n_grid + 1):
            ...
            # Compute per-input-channel scales from baseline-shifted L2 salience
            # Note: salience >= 1.0, so scales >= 1.0 for all α
    ...
    def quantize_layer(self, name, module):
        """
        Apply Group-Wise AWQ with Asymmetric Quantization and Baseline-Shifted L2 Salience.

        Steps:
        1. Grid search for best per-input-channel scales (E[1+X²]-based)
    ...
    def quantize_model(self):
        """Quantize all linear layers using Group-Wise AWQ with Baseline-Shifted L2 Salience."""
        print("\n" + "=" * 80)
        print("Quantizing with Group-Wise AWQ ASYMMETRIC + Baseline-Shifted L2 Salience")
        print("=" * 80)
        print("Method:")
        print("  1. Compute per-input-channel baseline-shifted L2 salience: s[j] = E[1 + X[:, j]²]")
        print("     → Guarantees minimum salience of 1.0 for all channels")
        ...
        print("\nKey Improvement: Baseline shift prevents zero salience")
        print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Group-Wise AWQ with ASYMMETRIC quantization and Baseline-Shifted L2 Salience for MiniCPM-2B",
    ...
    parser.add_argument("--output-dir", type=str, default="./quantized_models/minicpm_gw_awq_asym_1l2",
    ...
    print("=" * 80)
    print("Group-Wise AWQ with ASYMMETRIC Quantization + Baseline-Shifted L2 Salience")
    print("=" * 80)
    print("Algorithm:")
    print("  1. Per-input-channel baseline-shifted L2 salience: s[j] = E[1 + X[:, j]²]")
    print("     → Minimum salience of 1.0 prevents zero-scaling")
    ...
    print("\nKey Improvement: Baseline shift protects low-activity channels")
    print("=" * 80)
    ...
    quantizer = GroupWiseAWQAsymmetric1L2Quantizer(
        model=model,
        tokenizer=tokenizer,
        device=device,
        bits=4,
    ...
    print("\nGroup-Wise AWQ ASYMMETRIC + Baseline-Shifted L2 Approach:")
    print("  ✓ Baseline-shifted L2 salience: E[1+X²] (minimum = 1.0)")
    print("  ✓ Grid search for optimal α")
    print("  ✓ Column-wise weight scaling")
    print(f"  ✓ GROUP-WISE ASYMMETRIC quantization [0, 15]")
    print("  ✓ Better handling of low-activity channels")
    print("=" * 80)
