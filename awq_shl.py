...
class StandardHeuristicAWQQuantizerLeastFirst:
    """
    Standard Heuristic AWQ - LEAST-FIRST strategy (Fair comparison version).
    ...
    heuristic-guided quantization with LEAST-FIRST strategy.
    ...
    NEW CHANGE:
    - Flips from LEAST significant (low cost) to MOST significant (high cost)
    """
    ...
    def __init__(self, model, tokenizer, device="cuda", bits=4, n_grid=20,
        ...
        print(f"\n[Standard Heuristic AWQ Quantizer (LEAST-FIRST) Initialized]")
    ...
    def quantize_model(self):
        print("\n" + "=" * 80)
        print("Quantizing with Standard Heuristic AWQ (LEAST-FIRST)")
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./quantized_models/minicpm_awq_shl")
    ...
    quantizer = StandardHeuristicAWQQuantizerLeastFirst(
        model=model,
        tokenizer=tokenizer,
        device=device,
        bits=4,
